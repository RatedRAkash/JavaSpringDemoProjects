...
/**
 * A Time-based One-time Password (TOTP) builder. <p> This is an implementation of the OATH TOTP
 * algorithm as described by <a href="https://tools.ietf.org/html/rfc6238">RFC 6238</a>. This
 * implementation supports numeric-only TOTP values ranging from size 6 to 8 (inclusive). </p> <p>
 * The builder, obtained via a call to the static {@code key(...)} method on {@link TOTP}, provides
 * methods for configuring the TOTP generation parameters. Once the TOTP configuration is prepared,
 * the builder is used to generate a {@link TOTP} using the {@code build()} or {@code build(time)}
 * methods: </p>
 *
 * <pre>
...
public final class TOTPBuilder {
    ...
    /**
     * Creates a new instance of {@code TOTPBuilder} initialized with a shared secret key.
     *
     * @param key the shared secret key. The contents of the array are copied to protect against
    ...
    /**
     * Returns this {@code TOTPBuilder} instance initialized with the specified {@code timeStep}
     * size.
     *
     * @param timeStep the time step size in milliseconds
     * @return this {@code TOTPBuilder} instance initialized with the specified {@code timeStep}
     * size.
     * @throws IllegalArgumentException if {@code timeStep} is {@literal <=} 0.
     */
    ...
    /**
     * Returns this {@code TOTPBuilder} instance initialized with the specified {@code digits}.
     *
     * @param digits the number of digits the generated TOTP value should contain (must be between
     * {@link #MIN_ALLOWED_DIGITS} and {@link #MAX_ALLOWED_DIGITS} inclusive)
     * @return this {@code TOTPBuilder} instance initialized with the specified {@code digits}.
     * @throws IllegalArgumentException if {@code digits} is not in [{@link #MIN_ALLOWED_DIGITS},
     * {@link #MAX_ALLOWED_DIGITS}].
     */
    public TOTPBuilder digits(int digits) {
        Preconditions.checkArgument(
                Range.closed(MIN_ALLOWED_DIGITS, MAX_ALLOWED_DIGITS).contains(digits));
        this.digits = digits;
        return this;
    }

    /**
     * Returns this {@code TOTPBuilder} instance initialized with the specified HMAC-SHA {@code
     * algorithm}.
     *
     * @param algorithm the HMAC-SHA algorithm used in generating the TOTP value
     * @return this {@code TOTPBuilder} instance initialized with the specified HMAC-SHA {@code
     * algorithm}.
    ...
    /**
     * Returns this {@code TOTPBuilder} instance initialized with the {@link
     * HmacShaAlgorithm#HMAC_SHA_1}.
     *
     * @return this {@code TOTPBuilder} instance initialized with the {@link
     * HmacShaAlgorithm#HMAC_SHA_1}.
     */
    ...
    /**
     * Build a Time-based One-time Password {@link TOTP} using the current system time (current time
     * in milliseconds since the UNIX epoch). Note that the builder instance can be reused for
     * subsequent configuration/generation calls.
     *
    ...
    /**
     * Build a Time-based One-time Password {@link TOTP} using an arbitrary time. Note that the
     * builder instance can be reused for subsequent configuration/generation calls.
     *
     * @param time the time (in milliseconds) (must be {@literal >= 0})
     * @return a Time-based One-time Password {@link TOTP} instance.
     * @throws IllegalArgumentException if {@code time} {@literal <} 0.
     */
    ...
    /**
     * Returns the Time-based One-time Password value against an arbitrary {@code time} using the
     * set of parameters configured in this builder. The value will contain {@link #digits(int)}
     * digits.
     *
     * @param time the time (in milliseconds)
     * @return the Time-based One-time Password value as numeric String in base 10 that includes
     * {@link #digits(int)} digits.
     */
