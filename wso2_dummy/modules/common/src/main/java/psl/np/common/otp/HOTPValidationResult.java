...
public class HOTPValidationResult {
    ...
    /**
     * Creates a new instance of {@code HOTPValidationResult}. Note that all parameters are assumed
     * to be valid since the {@link HOTPValidator} is responsible for validating arguments, and
     * creation of {@link HOTPValidationResult}s.
     *
     * @param valid {@code true} if an HOTP authentication pass has succeeded or not {@code false}
     * @param newMovingFactor the new moving factor value that should be bound to the underlying
     * client upon successful validation. If the validation was unsuccessful, then the original
     ...
     * successful validation. If the validation was unsuccessful, then this must return the original
     * moving factor value.
     *
